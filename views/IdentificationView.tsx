...
import { identifyPlant } from '../geminiService';
...
const IdentificationView: React.FC<IdentificationViewProps> = ({ onResult, onAddReminder }) => {
  ...
  const getRemedyIcon = (remedy: string) => {
    ...
    if (r.includes('vinegar') || r.includes('liquid')) {
      return (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517" />
        </svg>
      );
    }
    return (
  ...
  if (result && capturedImage) {
    const careItems = [
      { 
        label: 'Hydration', 
        value: result.careGuide.watering, 
        icon: 'M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7', 
        color: 'text-blue-500', 
        bg: 'bg-blue-50',
        badge: getBadge(result.careGuide.watering, 'water')
      },
      { 
        label: 'Luminosity', 
        value: result.careGuide.light, 
        icon: 'M12 3v1m0 16v1m9-9h-1M4 9h-1', 
        color: 'text-amber-500', 
        bg: 'bg-amber-50',
        badge: getBadge(result.careGuide.light, 'light')
      },
      { 
        label: 'Substrate', 
        value: result.careGuide.soil, 
        icon: 'M3.055 11H5a2 2 0 012 2v1', 
        color: 'text-orange-700', 
        bg: 'bg-orange-50',
        badge: getBadge(result.careGuide.soil, 'soil')
      },
      { 
        label: 'Climate', 
        value: result.careGuide.humidity, 
        icon: 'M12 2.25c-5.385 0-9.75 4.365', 
        color: 'text-cyan-600', 
        bg: 'bg-cyan-50' 
      },
      { 
        label: 'Fertilization', 
        value: result.careGuide.fertilizer, 
          ...
          <div className="space-y-4 mt-2">
            {result.careGuide.homeRemedies.map((remedy, idx) => (
              <div key={idx} className="flex items-start bg-slate-50/50 p-4 rounded-2xl border border-slate-100 hover:bg-botanist/5 transition-colors group">
                <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-botanist shadow-sm mr-4 flex-shrink-0 group-hover:scale-110 transition-transform">
                  {getRemedyIcon(remedy)}
                </div>
                <div className="pt-1">
                  <p className="text-sm text-slate-700 font-bold leading-relaxed">{remedy}</p>
                </div>
              </div>
            ))}
          </div>
        ), 
        icon: 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.673.337a4 4 0 01-2.586.344l-2.096-.419a1 1 0 01-.801-1.223l.112-.556a3 3 0 01.18-.545l.135-.303a1 1 0 00-.13-.984l-1.033-1.378a1 1 0 00-1.428-.182', 
        color: 'text-botanist', 
        bg: 'bg-botanist/10' 
      }
    ];
      ...
      <div className="space-y-10 animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            <div 
              ref={containerRef}
              className="relative bg-white rounded-[3.5rem] overflow-hidden shadow-2xl border border-slate-50 cursor-move h-[500px]"
            ...
            >
              ...
              <div className="absolute bottom-6 right-6 flex flex-col space-y-3">
                <button onClick={handleZoomIn} className="w-12 h-12 bg-white/90 backdrop-blur-md rounded-2xl flex items-center justify-center text-slate-800 shadow-xl border border-white/50 hover:bg-white transition-colors">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M12 4v16m8-8H4" /></svg>
                </button>
                <button onClick={handleZoomOut} className="w-12 h-12 bg-white/90 backdrop-blur-md rounded-2xl flex items-center justify-center text-slate-800 shadow-xl border border-white/50 hover:bg-white transition-colors">
                ...
                <button onClick={handleResetZoom} className="w-12 h-12 bg-white/90 backdrop-blur-md rounded-2xl flex items-center justify-center text-slate-800 shadow-xl border border-white/50 hover:bg-white transition-colors">
              ...
              {result.isToxic && (
                <div className="absolute top-6 left-6 bg-red-500/90 backdrop-blur-md text-white px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-lg border border-white/20">
                  Toxic to Pets
                </div>
              )}
              {result.isWeed && (
                <div className="absolute top-6 right-6 bg-amber-500/90 backdrop-blur-md text-white px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-lg border border-white/20">
                  Weed Species
                </div>
              )}
            </div>
          ...
          <div className="space-y-8">
            ...
            {result.isToxic && (
              <div className="bg-red-50 p-8 rounded-[3rem] border border-red-100 flex items-start space-x-6">
                <div className="w-14 h-14 bg-red-100 rounded-2xl flex items-center justify-center text-red-600 shadow-inner flex-shrink-0">
                  <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4" /></svg>
                </div>
                <div>
                  <h4 className="text-red-900 font-black text-lg mb-2">Toxicity Alert</h4>
                  <p className="text-red-700/80 font-bold leading-relaxed">{result.toxicityDetails}</p>
                </div>
              </div>
            )}
          </div>
